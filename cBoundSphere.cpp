...
//월드 단위의 센터와 반지름을 얻는다.
void cBoundSphere::GetWorldCenterRadius( const  Transform* pTrans, D3DXVECTOR3* center, float* raidus )
{
...
//월드 단위로 그려진다.
void cBoundSphere::RenderGizmo( const  Transform* pTrans, DWORD color)
{
	D3DXVECTOR3 worldCenter;
	float radius;

	this->GetWorldCenterRadius( pTrans, &worldCenter,&radius );
	GIZMO_MGR->WireSphere( worldCenter, radius, color);
}
