...
LPD3DXEFFECT cTerrain::m_pTerrainEffect = NULL;
...
void cTerrain::Render(Camera * pCam)
{
	//월드 행렬셋팅
	...
	//광원 셋팅
	m_pTerrainEffect->SetVector("worldLightDir", &D3DXVECTOR4(0, -1, 0, 1));
	m_pTerrainEffect->SetVector("worldLightColor", &D3DXVECTOR4(1, 1, 1, 1));

	m_pTerrainEffect->SetVector("worldLightDir2", &D3DXVECTOR4(0, -1, 0, 1));
	m_pTerrainEffect->SetVector("worldLightColor2", &D3DXVECTOR4(1, 1, 1, 1));
...
//랜더
void cTerrain::Render( Camera* pCam, cLight_Direction* pDirectionLight, cLight_Direction* pDirectionLight2)
{
	//월드 행렬셋팅
	D3DXMATRIXA16 matInd;
	D3DXMatrixIdentity( &matInd );
	...
	m_pTerrainEffect->SetVector("worldLightColor", &pDirectionLight->GetColorRGBA());

	D3DXVECTOR3 dirLight2 = pDirectionLight2->pTransform->GetForward();
	m_pTerrainEffect->SetVector("worldLightDir2", &D3DXVECTOR4(dirLight2, 1));
	m_pTerrainEffect->SetVector("worldLightColor2", &pDirectionLight2->GetColorRGBA());
...
void cTerrain::Render( Camera* pCam, cLight_Direction* pDirectionLight, cLight_Direction* pDirectionLight2, Camera* pDirectionLightCamera )
{
...
void cTerrain::RenderShadow( Camera* pDirectionLightCam )
{
	//월드 행렬셋팅
	...
	m_pTerrainEffect->SetMatrix("matLightViewProjection", &pDirectionLightCam->GetViewProjectionMatrix());
...
void cTerrain::SetCamera(const Camera* camera)
{
	if (!m_pTerrainEffect) return;

	D3DXMATRIXA16 matViewProj = camera->GetViewProjectionMatrix();
	m_pTerrainEffect->SetMatrix("matViewProjection", &matViewProj);

	D3DXVECTOR3 vEyePos = camera->GetWorldPosition();

	m_pTerrainEffect->SetVector("vEyePos", &D3DXVECTOR4(vEyePos, 1));

	m_pTerrainEffect->SetFloat("camNear", camera->GetNear());
	m_pTerrainEffect->SetFloat("camFar", camera->GetFar());
}
...
/*
//technique 설정
void cTerrain::SetTechniqueName(std::string name)
{
	if (!m_pTerrainEffect) return;
	m_pTerrainEffect->SetTechnique(name.c_str());
}
*/