...
void Transform::LookDirection(const D3DXVECTOR3 & dir, Transform * p, const D3DXVECTOR3 & up)
{
	//정면 벡터
	D3DXVECTOR3 newForward = dir;
	...
	if (p)
	{
		//새로운 축 성분에 부모 역행렬 곱해....
		D3DXMATRIXA16 matInvParentFinal;
		D3DXMatrixInverse(&matInvParentFinal, NULL, &p->matFinal);
...
void Transform::LookPosition(const D3DXVECTOR3 & pos, Transform * p, const D3DXVECTOR3 & up)
{
	//위치에 대한 방향벡터를 얻는다.
	if (p == NULL) LookPosition(pos, up);

	D3DXVECTOR3 worldPos = this->GetWorldPosition();
	if(p) worldPos += p->GetWorldPosition();
	D3DXVECTOR3 dir = pos - worldPos;

	D3DXVec3Normalize(&dir, &dir);
	this->LookDirection(dir, p, up);
}
...
void Transform::ShakeUpdate(float timeDelta)
{
	...
	if( this->ShakePosPower > 0.0f && this->ShakePosFlag != 0 )
	{
		...
		this->SetLocalPosition(GetWorldPosition() + localPos );
