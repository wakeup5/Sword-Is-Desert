...
void cXMesh_Static::Render( const Transform* Trans )
{
	//월드행렬 셋팅
	...
	//Effect 로 그리기 시작
	//sStaticMeshEffect->SetTechnique( "Base" );
...
void cXMesh_Static::RenderShadow(const Transform * Trans, Camera * light)
{
	//월드행렬 셋팅
	D3DXMATRIXA16 matWorld = Trans->GetFinalMatrix();
	sStaticMeshEffect->SetMatrix("matWorld", &matWorld);
	...
	sStaticMeshEffect->SetMatrix("matViewProjection", &light->GetViewProjectionMatrix());

	//Effect 로 그리기 시작
	sStaticMeshEffect->SetTechnique("CreateShadow");
...
void cXMesh_Static::SetCamera( const Camera* camera )
{
	if (!sStaticMeshEffect) return;
	...
	sStaticMeshEffect->SetFloat("camNear", camera->GetNear());
	sStaticMeshEffect->SetFloat("camFar", camera->GetFar());
}
