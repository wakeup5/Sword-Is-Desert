...
void cXMesh_Skinned::Update(float timeDelta)
{
	...
	manimationPlayFactor = m_Track_Desc_0.Position / m_pNowPlayAnimationSet->GetPeriod();

	//마지막에 도달했다면...
	if (manimationPlayFactor >= 1.0)
	{
	...
	manimationPlayFactor = manimationPlayFactor - (int)manimationPlayFactor;	//정수부분 안생기게....
...
void cXMesh_Skinned::Render(const Transform* pTrans)
{

	D3DXMATRIXA16 matFinal, matFinal2;

	//Transform* temp = new Transform;
	//*temp = *pTrans;

	//memcpy(temp, pTrans, sizeof(Transform));

	//Transform 이 들어왔다면...
	if (pTrans != NULL)
	{
		//pTrans->RotateWorld(0, RAD(180), 0);
		D3DXMATRIXA16 matWorld = pTrans->GetFinalMatrix();
		matFinal = m_matCorrection * matWorld;
		//matFinal2 = m_matCorrection * pTrans->GetFinalMatrix();
	}
	else
	{
...
//본을 보여준다.
void cXMesh_Skinned::RenderBoneName(Camera* pCam, Transform* transform)
{
	...
	if (pBone->pMeshContainer)
	{
		...
		if (pBoneMesh->BufBoneCombos != NULL)
		{
			...
			for (DWORD i = 0; i < pBoneMesh->NumAttributesGroup; i++)
			{
				...
				m_pSkinnedEffect->SetTexture("m_Tex", pBoneMesh->DiffuseTexs[materialIndex]);
...
void cXMesh_Skinned::RenderBasic(BONE* pBone)
{
	//본에 메쉬 컨테이너가 존재한다면 그려야한다.
	if (pBone->pMeshContainer)
	{
		...
		for (DWORD i = 0; i < pBoneMesh->NumAttributesGroup; i++)
		{
			...
			DEVICE->SetTransform(D3DTS_WORLD, &m_pmWorkingPalette[0]);

			if(pBoneMesh->WorkingMesh) pBoneMesh->WorkingMesh->DrawSubset(i);
		}
	}

	//이웃 본이 존재한다면...
	if (pBone->pFrameSibling)
		RenderBasic((BONE*)pBone->pFrameSibling);

	//자식 본이 존재한다면...
	if (pBone->pFrameFirstChild)
		RenderBasic((BONE*)pBone->pFrameFirstChild);
}


//본이름을 출력한다.
void cXMesh_Skinned::RenderChildBoneName(Camera* pCam, BONE* pBone, D3DXVECTOR3* parentPos)
{
	if (pBone == NULL) return;
...
void cXMesh_Skinned::SetCamera(Camera* Camera)
{
	if (!sSkinnedMeshEffect) return;
	...
	sSkinnedMeshEffect->SetFloat("camFar", Camera->GetFar());
	sSkinnedMeshEffect->SetFloat("camNear", Camera->GetNear());
