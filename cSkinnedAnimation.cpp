...
void		cSkinnedAnimation::Render(Transform* pTransform )
{
...
void		cSkinnedAnimation::RenderBoneName(Camera* pCam, Transform* pTransform )
{
	//현재 자신의 Animation 정보로 셋팅
	m_pAnimController->AdvanceTime( m_fAnimDelta, NULL );
	m_fAnimDelta = 0.0f;		//애니메이션을 진행시켰기 때문에 delta 량은 0 으로....
...
void cSkinnedAnimation::AddBoneTransform( std::string boneName, Transform* pTransform  )
{
	BONE* pBone = this->m_pSkinnedMesh->GetFineBONE( boneName );
	this->m_mapBoneTransform.insert( std::make_pair( pBone, pTransform ) );
}

void cSkinnedAnimation::AddApplyTransform( std::string boneName, Transform* pTransform )
{
	BONE* pBone = this->m_pSkinnedMesh->GetFineBONE( boneName );
